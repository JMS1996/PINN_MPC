#!/usr/bin/env python3
"""
PX4 Phase 1 dataset builder, v1.

Input:
    px4_phase1_v12 run directory containing summary.csv and per-scenario
    telemetry_analysis.csv files.

Output:
    Colab-ready one-step dynamics CSVs:
...
def latest_run_from_glob(pattern: str) -> Path:
    import glob
...
def make_output_dir(config: dict[str, Any], output_root: str | None) -> Path:
    ...
    name = config.get("dataset_name", "px4_phase1_rate_dynamics_dataset_v1")
...
def main() -> None:
    ...
    parser.add_argument("--run-dir", default=None, help="Specific px4_phase1_v12 run directory.")
    parser.add_argument("--output-root", default=None)
    args = parser.parse_args()

    config_path = expand_path(args.config)
    config = load_config(config_path)
    run_dir = expand_path(args.run_dir) if args.run_dir else latest_run_from_glob(config["source_run_glob"])
    ...
    for sdir in scenario_dirs(run_dir):
        ...
        samples, report = build_samples_for_scenario(sdir, summary_row, state_cols, action_cols, filters)
