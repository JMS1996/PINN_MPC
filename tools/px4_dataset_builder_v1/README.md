# PX4 Dataset Builder v1

Builds the first supervised dynamics dataset from `px4_phase1_v12` telemetry.
...
```bash
cd /mnt/c/Users/jangm/Documents/New\ project/drive_upload_code/PINN_MPC/tools/px4_dataset_builder_v1
python3 build_px4_dataset_v1.py --config config.json
```

Use a specific run:

```bash
python3 build_px4_dataset_v1.py \
  --config config.json \
...
```text
~/px4_datasets/processed/px4_phase1_rate_dynamics_dataset_v1_<timestamp>/
```
