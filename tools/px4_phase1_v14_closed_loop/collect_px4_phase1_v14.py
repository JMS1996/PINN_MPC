#!/usr/bin/env python3
"""
PX4 Gazebo Phase 1 data collector, v14 closed-loop.
...
def normalize_position_setpoint(sp: dict[str, Any], default_down_m: float) -> dict[str, Any]:
    return {
        ...
        "yaw_deg": float(sp.get("yaw_deg", 0.0)),
        "hold_s": float(sp.get("hold_s", 1.0)),
    }


def normalize_velocity_setpoint(sp: dict[str, Any], default_down_m_s: float = 0.0) -> dict[str, Any]:
    return {
...
async def position_setpoint_driver(
    ...
    setpoints = [
        normalize_position_setpoint(sp, default_down_m)
    ...
    for sp in setpoints:
        ...
        print(
            ...
            f"D={sp['down_m']:.2f} yaw={sp['yaw_deg']:.1f} hold={sp['hold_s']:.1f}s"
        )
        end_t = time.time() + sp["hold_s"]
        while time.time() < end_t and not stop_event.is_set():
...
async def velocity_setpoint_driver(
    ...
    setpoints = [
        normalize_velocity_setpoint(sp)
    ...
    for sp in setpoints:
        ...
        print(
            ...
            f"VD={sp['down_m_s']:.2f} yaw={sp['yaw_deg']:.1f} hold={sp['hold_s']:.1f}s"
        )
        end_t = time.time() + sp["hold_s"]
        while time.time() < end_t and not stop_event.is_set():
...
async def main_async(args: argparse.Namespace) -> None:
    ...
    run_metadata = {
        "collector_version": "v14_closed_loop",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="PX4 Gazebo Phase 1 data collector v14 closed-loop.")
