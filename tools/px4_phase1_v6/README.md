# PX4 Phase 1 Rate-Setpoint Collection v6

Version `v6` fixes the main issue found in `v5`: safety started before the vehicle reached takeoff altitude, so calibration runs aborted around 5 m.
...
```bash
cd /mnt/c/Users/jangm/Documents/New\ project/drive_upload_code/PINN_MPC/tools/px4_phase1_v6
python3 collect_px4_phase1_v6.py --config scenarios.yaml --scenario T0_thrust_cal_032
python3 collect_px4_phase1_v6.py --config scenarios.yaml --scenario T1_thrust_cal_036
python3 collect_px4_phase1_v6.py --config scenarios.yaml --scenario T2_thrust_cal_040
python3 collect_px4_phase1_v6.py --config scenarios.yaml --scenario T3_thrust_cal_044
```
...
runs = sorted(root.glob('px4_phase1_standard_vtol_rate_thrust_safe_v6_*'))
rows = []
for run in runs[-10:]:
    p = run / 'summary.csv'
    if p.exists():
...
```bash
python3 collect_px4_phase1_v6.py --config scenarios.yaml --scenario R1_pitch_rate_doublet_safe
```
