#!/usr/bin/env python3
"""
PX4 Phase 1 dataset builder, v4.

Input:
    One or more px4_phase1_v13 run directories containing summary.csv and
...
Output:
    ...
    plus auxiliary acceleration targets from v13:
        aux_derived_accel_*, aux_derived_*_accel_rad_s2

v4 adds conservative one-step outlier filters and allows hard scenarios such as
N11_mixed_sequence to be partially included in training instead of becoming a
pure out-of-distribution test set.
...
def make_output_dir(config: dict[str, Any], output_root: str | None) -> Path:
    ...
    name = config.get("dataset_name", "px4_phase1_rate_dynamics_dataset_v4_filtered_imu_aux")
...
def deterministic_row_split(df: pd.DataFrame, config: dict[str, Any]) -> pd.DataFrame:
    ...
    # Hard scenarios are useful for testing, but making N11 purely OOD made the
    # rollout test harsher than the intended Phase 1 local-model objective. v4
    # keeps a holdout fraction and lets the rest join train/val.
    hard_holdout_idx: list[int] = []
    for scenario in sorted(test_scenarios):
        idx = out.index[out["scenario"].eq(scenario)].to_numpy()
        if not len(idx):
            continue
        shuffled = idx.copy()
        rng.shuffle(shuffled)
        n_test = max(1, int(round(len(shuffled) * hard_test_fraction)))
        hard_holdout_idx.extend(shuffled[:n_test].tolist())
    if hard_holdout_idx:
        out.loc[hard_holdout_idx, "split"] = "test"

    remaining_mask = out["split"].eq("train") & ~out["scenario"].isin(val_scenarios)
...
def main() -> None:
    ...
    parser.add_argument("--run-dir", action="append", default=None, help="Specific px4_phase1_v13 run directory. Can be repeated.")
