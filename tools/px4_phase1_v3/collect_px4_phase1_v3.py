#!/usr/bin/env python3
"""
PX4 Gazebo Phase 1 data collector, v3.

Scope:
- Launch a fresh PX4/Gazebo SITL process for each scenario.
- Connect through MAVSDK.
- Run nominal offboard maneuver setpoint sequences.
...
async def watch_position_velocity_ned(drone: System, state: TelemetryState) -> None:
    async for pv in drone.telemetry.position_velocity_ned():
        state.local_north_m = float(pv.position.north_m)
        state.local_east_m = float(pv.position.east_m)
        state.local_down_m = float(pv.position.down_m)
        state.vel_north_m_s = float(pv.velocity.north_m_s)
        state.vel_east_m_s = float(pv.velocity.east_m_s)
        state.vel_down_m_s = float(pv.velocity.down_m_s)
...
def normalize_setpoint(sp: dict[str, Any]) -> dict[str, Any]:
    return {
        "label": str(sp.get("label", "setpoint")),
        "north_m": float(sp.get("north_m", 0.0)),
        "east_m": float(sp.get("east_m", 0.0)),
        "altitude_m": float(sp.get("altitude_m", 25.0)),
        "yaw_deg": float(sp.get("yaw_deg", 0.0)),
        "hold_s": float(sp.get("hold_s", 5.0)),
    }
...
def update_ref_state(ref_state: dict[str, Any], sp: dict[str, Any], phase: str) -> None:
    ref_state.update({
        ...
        "north_m": sp["north_m"],
        "east_m": sp["east_m"],
        "altitude_m": sp["altitude_m"],
        "yaw_deg": sp["yaw_deg"],
    })


async def offboard_setpoint_driver(
    drone: System,
    scenario: dict[str, Any],
    ref_state: dict[str, Any],
    stop_event: asyncio.Event,
) -> None:
    setpoints = [normalize_setpoint(sp) for sp in scenario.get("setpoints", [])]
    if not setpoints:
        setpoints = [normalize_setpoint({
            "label": "default_hold",
            "north_m": 0.0,
            "east_m": 0.0,
            "altitude_m": scenario.get("takeoff_altitude_m", 25.0),
            "yaw_deg": 0.0,
    ...
    for sp in setpoints:
        last_sp = sp
        update_ref_state(ref_state, sp, "offboard_maneuver")
        print(
            f"[offboard] {sp['label']} N={sp['north_m']:.1f} E={sp['east_m']:.1f} "
            f"alt={sp['altitude_m']:.1f} yaw={sp['yaw_deg']:.1f} hold={sp['hold_s']:.1f}s"
        )
    ...
    update_ref_state(ref_state, last_sp, "offboard_final_hold")
...
async def collect_one_scenario(
    ...
    try:
        ...
        watchers = [
            ...
            asyncio.create_task(watch_position_velocity_ned(drone, state)),
        ...
        ref_state: dict[str, Any] = {
            ...
            "north_m": 0.0,
            "east_m": 0.0,
            "altitude_m": float(scenario.get("takeoff_altitude_m", 25.0)),
            "yaw_deg": 0.0,
        }
        stop_offboard = asyncio.Event()
        offboard_task: asyncio.Task | None = None
        if scenario.get("maneuver_type", "") == "offboard_position_ned":
            offboard_task = asyncio.create_task(
                offboard_setpoint_driver(drone, scenario, ref_state, stop_offboard)
            )
            await asyncio.sleep(0.5)
            if offboard_task.done():
                result = offboard_task.result()
...
def compute_quality_metrics(raw_df: pd.DataFrame, analysis_df: pd.DataFrame, scenario: dict[str, Any]) -> dict[str, Any]:
    ...
    local_n = finite_series(analysis_df, "local_north_m")
    local_e = finite_series(analysis_df, "local_east_m")
    ...
    ref_alt = finite_series(analysis_df, "ref_altitude_m")
    ref_n = finite_series(analysis_df, "ref_north_m")
    ref_e = finite_series(analysis_df, "ref_east_m")
    ref_alt_error = rel_alt.reset_index(drop=True) - ref_alt.reset_index(drop=True) if len(rel_alt) and len(ref_alt) == len(rel_alt) else pd.Series(dtype=float)
    ref_n_error = local_n.reset_index(drop=True) - ref_n.reset_index(drop=True) if len(local_n) and len(ref_n) == len(local_n) else pd.Series(dtype=float)
    ref_e_error = local_e.reset_index(drop=True) - ref_e.reset_index(drop=True) if len(local_e) and len(ref_e) == len(local_e) else pd.Series(dtype=float)
    if len(ref_n_error) and len(ref_e_error):
        horizontal_ref_error = np.sqrt(ref_n_error.to_numpy() ** 2 + ref_e_error.to_numpy() ** 2)
    else:
        horizontal_ref_error = np.array([])
    ...
    metrics = {
        ...
        "analysis_ref_altitude_rmse_m": safe_float(np.sqrt(np.mean(np.square(ref_alt_error)))) if len(ref_alt_error) else None,
        "analysis_ref_horizontal_rmse_m": safe_float(np.sqrt(np.mean(np.square(horizontal_ref_error)))) if len(horizontal_ref_error) else None,
        ...
        "analysis_final_local_north_m": safe_float(local_n.iloc[-1]) if len(local_n) else None,
        "analysis_final_local_east_m": safe_float(local_e.iloc[-1]) if len(local_e) else None,
        "analysis_final_ref_north_m": safe_float(ref_n.iloc[-1]) if len(ref_n) else None,
        "analysis_final_ref_east_m": safe_float(ref_e.iloc[-1]) if len(ref_e) else None,
        "analysis_final_ref_altitude_m": safe_float(ref_alt.iloc[-1]) if len(ref_alt) else None,
...
def plot_quicklook(raw_df: pd.DataFrame, analysis_df: pd.DataFrame, scenario: dict[str, Any], path: Path) -> None:
    ...
    if "ref_altitude_m" in raw_df:
        axes[0].plot(raw_df["time_s"], pd.to_numeric(raw_df["ref_altitude_m"], errors="coerce"), "k--", lw=0.9, label="ref")
        axes[0].legend(loc="best")
    elif np.isfinite(target_alt):
        axes[0].axhline(target_alt, color="k", ls="--", lw=0.9, label="target")

    plot_col(axes[1], "pitch_deg", "Pitch (deg)")
    plot_col(axes[2], "local_north_m", "North (m)")
    if "ref_north_m" in raw_df:
        axes[2].plot(raw_df["time_s"], pd.to_numeric(raw_df["ref_north_m"], errors="coerce"), "k--", lw=0.9, label="ref")
        axes[2].legend(loc="best")
    plot_col(axes[3], "local_east_m", "East (m)")
    if "ref_east_m" in raw_df:
        axes[3].plot(raw_df["time_s"], pd.to_numeric(raw_df["ref_east_m"], errors="coerce"), "k--", lw=0.9, label="ref")
        axes[3].legend(loc="best")
    plot_col(axes[4], "vel_down_m_s", "Down Vel (m/s)")
