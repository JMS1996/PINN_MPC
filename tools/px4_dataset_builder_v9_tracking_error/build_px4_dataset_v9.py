#!/usr/bin/env python3
"""
PX4 Phase 1 dataset builder, v9 tracking-error position-state block split.
...
v9 keeps the v8 position-state contract and adds explicit tracking-error
features:
    e_ref_north_m, e_ref_east_m, e_ref_alt_m, e_ref_yaw_deg,
    e_ref_vn_m_s, e_ref_ve_m_s, e_ref_vd_m_s

These features make disturbance/recovery regimes explicit to the learned model:
the setpoint can remain fixed while the current state is displaced.
...
def make_output_dir(config: dict[str, Any], output_root: str | None) -> Path:
    ...
    name = config.get("dataset_name", "px4_phase1_recovery_tracking_error_dataset_v9")
...
def wrap_deg_np(values: np.ndarray) -> np.ndarray:
...
def build_samples_for_scenario(
    ...
    for col in action_cols:
        ...
        du = values - prev_values
        if col in {"ref_yaw_deg", "ref_yaw_offset_deg"}:
            du = wrap_deg_np(du)
        out[f"du_{col}"] = du

    # Explicit tracking-error features. These are not targets; they are inputs
    # that tell the model how far the current state is from the active PX4
    # setpoint. That distinction matters during disturbance recovery: the
    # setpoint can stay fixed while the state is displaced.
    out["e_ref_north_m"] = out["u_ref_north_m"] - out["x_local_north_m"]
    out["e_ref_east_m"] = out["u_ref_east_m"] - out["x_local_east_m"]
    out["e_ref_alt_m"] = (-out["u_ref_down_m"]) - out["x_relative_altitude_m"]
    out["e_ref_yaw_deg"] = wrap_deg_np(out["u_ref_yaw_deg"].to_numpy() - out["x_yaw_deg"].to_numpy())
    out["e_ref_vn_m_s"] = out["u_ref_north_m_s"] - out["x_vel_north_m_s"]
    out["e_ref_ve_m_s"] = out["u_ref_east_m_s"] - out["x_vel_east_m_s"]
    out["e_ref_vd_m_s"] = out["u_ref_down_m_s"] - out["x_vel_down_m_s"]
...
def main() -> None:
    ...
    write_json(
        ...
        {
            "builder_version": "v9_tracking_error_position_state_block_split",
            "source_run_dirs": [str(p) for p in run_dirs],
            "output_dir": str(output_dir),
            "config_path": str(config_path),
            "state_cols": state_cols,
            "action_cols": action_cols,
            "feature_prefixes": ["x_", "u_", "prev_u_", "du_", "e_"],
            "error_cols": DEFAULT_ERROR_COLS,
            "target_prefix": "dx_",
