#!/usr/bin/env python3
"""
PX4 Gazebo Phase 1 data collector, v16 recovery tracking-error.

Scope:
- Launch a fresh PX4/Gazebo SITL process for each scenario.
- Connect through MAVSDK.
- Wait for takeoff altitude, then run PX4 closed-loop position/velocity offboard scenarios.
- Interpret scenario yaw commands as offsets from the vehicle yaw at offboard start.
- Collect recovery regimes where the vehicle is intentionally displaced from
  the nominal hold setpoint, then PX4 is commanded back to the nominal setpoint.
- Save raw telemetry, analysis-window telemetry, quality metrics, metadata, and plots.
- Add IMU acceleration and derived acceleration/rate-derivative columns for model diagnostics.
...
from mavsdk.offboard import AttitudeRate, OffboardError, PositionNedYaw, VelocityNedYaw
...
class TelemetryState:
    ...
    imu_accel_forward_m_s2: float | None = None
    imu_accel_right_m_s2: float | None = None
    imu_accel_down_m_s2: float | None = None
    imu_angular_rate_forward_rad_s: float | None = None
    imu_angular_rate_right_rad_s: float | None = None
    imu_angular_rate_down_rad_s: float | None = None
...
def _nested_attr(obj: Any, names: list[str], default: Any = None) -> Any:
    cur = obj
    for name in names:
...
async def watch_imu(drone: System, state: TelemetryState) -> None:
    """Best-effort MAVSDK IMU watcher.

    MAVSDK generated field names can vary slightly by version, so this watcher
    uses defensive nested attribute reads. If `telemetry.imu()` is unavailable,
    the task exits and these columns remain NaN.
    """
    if not hasattr(drone.telemetry, "imu"):
        print("[warn] MAVSDK telemetry.imu() not available; IMU columns will be empty")
        return
    async for imu in drone.telemetry.imu():
        state.imu_accel_forward_m_s2 = _nested_attr(imu, ["acceleration_frd", "forward_m_s2"])
        state.imu_accel_right_m_s2 = _nested_attr(imu, ["acceleration_frd", "right_m_s2"])
        state.imu_accel_down_m_s2 = _nested_attr(imu, ["acceleration_frd", "down_m_s2"])
        state.imu_angular_rate_forward_rad_s = _nested_attr(imu, ["angular_velocity_frd", "forward_rad_s"])
        state.imu_angular_rate_right_rad_s = _nested_attr(imu, ["angular_velocity_frd", "right_rad_s"])
        state.imu_angular_rate_down_rad_s = _nested_attr(imu, ["angular_velocity_frd", "down_rad_s"])
...
def resolve_yaw_deg(sp: dict[str, Any], yaw_base_deg: float, yaw_reference_mode: str) -> tuple[float, float]:
    if "yaw_offset_deg" in sp:
        offset = float(sp.get("yaw_offset_deg", 0.0))
    else:
        # In relative mode, legacy yaw_deg fields are treated as offsets so v15
        # does not accidentally command the aircraft to absolute north.
        offset = float(sp.get("yaw_deg", 0.0))
    if yaw_reference_mode == "absolute":
        yaw_abs = float(sp.get("yaw_deg", offset))
        offset = wrap_deg(yaw_abs - yaw_base_deg)
    else:
        yaw_abs = wrap_deg(yaw_base_deg + offset)
    return yaw_abs, offset


def normalize_position_setpoint(
    sp: dict[str, Any],
    default_down_m: float,
    yaw_base_deg: float = 0.0,
    yaw_reference_mode: str = "relative_to_start",
) -> dict[str, Any]:
    yaw_abs, yaw_offset = resolve_yaw_deg(sp, yaw_base_deg, yaw_reference_mode)
    return {
        "label": str(sp.get("label", "position_setpoint")),
        "north_m": float(sp.get("north_m", 0.0)),
        "east_m": float(sp.get("east_m", 0.0)),
        "down_m": float(sp.get("down_m", default_down_m)),
        "yaw_deg": yaw_abs,
...
def normalize_velocity_setpoint(
    sp: dict[str, Any],
    default_down_m_s: float = 0.0,
    yaw_base_deg: float = 0.0,
    yaw_reference_mode: str = "relative_to_start",
) -> dict[str, Any]:
    yaw_abs, yaw_offset = resolve_yaw_deg(sp, yaw_base_deg, yaw_reference_mode)
    return {
        "label": str(sp.get("label", "velocity_setpoint")),
        "north_m_s": float(sp.get("north_m_s", 0.0)),
        "east_m_s": float(sp.get("east_m_s", 0.0)),
        "down_m_s": float(sp.get("down_m_s", default_down_m_s)),
        "yaw_deg": yaw_abs,
        "yaw_offset_deg": yaw_offset,
        "yaw_base_deg": float(yaw_base_deg),
        "hold_s": float(sp.get("hold_s", 1.0)),
...
def update_position_ref_state(ref_state: dict[str, Any], sp: dict[str, Any], phase: str) -> None:
    ref_state.update({
        "phase": phase,
        "label": sp["label"],
        "north_m": sp["north_m"],
        ...
        "down_m": sp["down_m"],
        "yaw_deg": sp["yaw_deg"],
        "yaw_offset_deg": sp["yaw_offset_deg"],
        "yaw_base_deg": sp["yaw_base_deg"],
        "roll_rate_deg_s": 0.0,
...
def update_velocity_ref_state(ref_state: dict[str, Any], sp: dict[str, Any], phase: str) -> None:
    ref_state.update({
        "phase": phase,
        "label": sp["label"],
        "north_m_s": sp["north_m_s"],
        "east_m_s": sp["east_m_s"],
        "down_m_s": sp["down_m_s"],
        "yaw_deg": sp["yaw_deg"],
        "yaw_offset_deg": sp["yaw_offset_deg"],
        "yaw_base_deg": sp["yaw_base_deg"],
        "roll_rate_deg_s": 0.0,
...
async def send_position_setpoint(drone: System, sp: dict[str, Any]) -> None:
    await drone.offboard.set_position_ned(
        PositionNedYaw(sp["north_m"], sp["east_m"], sp["down_m"], sp["yaw_deg"])
    )


async def send_velocity_setpoint(drone: System, sp: dict[str, Any]) -> None:
    await drone.offboard.set_velocity_ned(
        VelocityNedYaw(sp["north_m_s"], sp["east_m_s"], sp["down_m_s"], sp["yaw_deg"])
    )
...
async def position_setpoint_driver(
    drone: System,
    scenario: dict[str, Any],
    ref_state: dict[str, Any],
    stop_event: asyncio.Event,
) -> None:
    default_down_m = -float(scenario.get("takeoff_altitude_m", 25.0))
    yaw_base_deg = float(ref_state.get("yaw_base_deg", 0.0))
    yaw_reference_mode = str(scenario.get("yaw_reference_mode", "relative_to_start"))
    setpoints = [
        normalize_position_setpoint(sp, default_down_m, yaw_base_deg, yaw_reference_mode)
        for sp in scenario.get("position_setpoints", [])
    ]
    if not setpoints:
        setpoints = [normalize_position_setpoint({
            "label": "default_position_hold",
            "down_m": default_down_m,
            "hold_s": scenario.get("duration_s", 30.0),
        }, default_down_m, yaw_base_deg, yaw_reference_mode)]

    rate_hz = float(scenario.get("setpoint_rate_hz", 20.0))
    dt = 1.0 / max(rate_hz, 1.0)
    initial_sp = setpoints[0]
    update_position_ref_state(ref_state, initial_sp, "offboard_initializing")
    await send_position_setpoint(drone, initial_sp)

    if scenario.get("start_offboard_after_takeoff", True):
        print("[offboard] start position-NED mode")
        try:
            await drone.offboard.start()
        except OffboardError as exc:
            raise RuntimeError(f"Offboard start failed: {exc}") from exc
    ...
    initial_hold_s = float(scenario.get("offboard_initial_hold_s", 0.0))
    if initial_hold_s > 0.0:
        update_position_ref_state(ref_state, initial_sp, "offboard_initial_hold")
        end_t = time.time() + initial_hold_s
        while time.time() < end_t and not stop_event.is_set():
            await send_position_setpoint(drone, initial_sp)
            await asyncio.sleep(dt)
    ...
    for sp in setpoints:
        last_sp = sp
        update_position_ref_state(ref_state, sp, "position_maneuver")
        print(
            f"[position] {sp['label']} N={sp['north_m']:.2f} E={sp['east_m']:.2f} "
            f"D={sp['down_m']:.2f} yaw={sp['yaw_deg']:.1f} "
            f"(offset={sp['yaw_offset_deg']:.1f}) hold={sp['hold_s']:.1f}s"
        )
        end_t = time.time() + sp["hold_s"]
        while time.time() < end_t and not stop_event.is_set():
            await send_position_setpoint(drone, sp)
            await asyncio.sleep(dt)

    update_position_ref_state(ref_state, last_sp, "position_final_hold")
    while not stop_event.is_set():
        await send_position_setpoint(drone, last_sp)
        await asyncio.sleep(dt)


async def velocity_setpoint_driver(
    drone: System,
    scenario: dict[str, Any],
    ref_state: dict[str, Any],
    stop_event: asyncio.Event,
    ...
    yaw_reference_mode = str(scenario.get("yaw_reference_mode", "relative_to_start"))
    setpoints = [
        normalize_velocity_setpoint(sp, yaw_base_deg=yaw_base_deg, yaw_reference_mode=yaw_reference_mode)
        for sp in scenario.get("velocity_setpoints", [])
    ]
    if not setpoints:
        setpoints = [normalize_velocity_setpoint({
            "label": "default_velocity_hold",
            "hold_s": scenario.get("duration_s", 30.0),
        }, yaw_base_deg=yaw_base_deg, yaw_reference_mode=yaw_reference_mode)]

    rate_hz = float(scenario.get("setpoint_rate_hz", 20.0))
    dt = 1.0 / max(rate_hz, 1.0)
    initial_sp = setpoints[0]
    update_velocity_ref_state(ref_state, initial_sp, "offboard_initializing")
    await send_velocity_setpoint(drone, initial_sp)

    if scenario.get("start_offboard_after_takeoff", True):
        print("[offboard] start velocity-NED mode")
        try:
            await drone.offboard.start()
        except OffboardError as exc:
            raise RuntimeError(f"Offboard start failed: {exc}") from exc
    ...
    initial_hold_s = float(scenario.get("offboard_initial_hold_s", 0.0))
    if initial_hold_s > 0.0:
        update_velocity_ref_state(ref_state, initial_sp, "offboard_initial_hold")
        end_t = time.time() + initial_hold_s
        while time.time() < end_t and not stop_event.is_set():
            await send_velocity_setpoint(drone, initial_sp)
            await asyncio.sleep(dt)
    ...
    for sp in setpoints:
        last_sp = sp
        update_velocity_ref_state(ref_state, sp, "velocity_maneuver")
        print(
            f"[velocity] {sp['label']} VN={sp['north_m_s']:.2f} VE={sp['east_m_s']:.2f} "
            f"VD={sp['down_m_s']:.2f} yaw={sp['yaw_deg']:.1f} "
            f"(offset={sp['yaw_offset_deg']:.1f}) hold={sp['hold_s']:.1f}s"
        )
        end_t = time.time() + sp["hold_s"]
        while time.time() < end_t and not stop_event.is_set():
            await send_velocity_setpoint(drone, sp)
            await asyncio.sleep(dt)

    update_velocity_ref_state(ref_state, last_sp, "velocity_final_hold")
    while not stop_event.is_set():
        await send_velocity_setpoint(drone, last_sp)
        await asyncio.sleep(dt)
...
async def collect_one_scenario(
    ...
    try:
        ...
        watchers = [
            ...
            asyncio.create_task(watch_imu(drone, state)),
        ...
        ref_state: dict[str, Any] = {
            ...
            "north_m": 0.0,
            "east_m": 0.0,
            "down_m": -float(scenario.get("takeoff_altitude_m", 25.0)),
            "north_m_s": 0.0,
            "east_m_s": 0.0,
            "down_m_s": 0.0,
            "yaw_deg": wrap_deg(float(state.yaw_deg or 0.0)),
            "yaw_offset_deg": 0.0,
            "yaw_base_deg": wrap_deg(float(state.yaw_deg or 0.0)),
        }
        print(f"[{name}] yaw base for relative setpoints: {ref_state['yaw_base_deg']:.2f} deg")
        stop_offboard = asyncio.Event()
        offboard_task: asyncio.Task | None = None
        maneuver_type = scenario.get("maneuver_type", "")
        if maneuver_type == "offboard_attitude_rate":
            offboard_task = asyncio.create_task(
                rate_setpoint_driver(drone, scenario, ref_state, stop_offboard)
            )
        elif maneuver_type == "offboard_position_ned":
            offboard_task = asyncio.create_task(
                position_setpoint_driver(drone, scenario, ref_state, stop_offboard)
            )
        elif maneuver_type == "offboard_velocity_ned":
            offboard_task = asyncio.create_task(
                velocity_setpoint_driver(drone, scenario, ref_state, stop_offboard)
            )
        elif maneuver_type:
            raise ValueError(f"Unsupported maneuver_type: {maneuver_type}")
        ...
        raw_df = add_derived_dynamics(pd.DataFrame(rows))
...
def add_derived_dynamics(df: pd.DataFrame) -> pd.DataFrame:
    """Add finite-difference accelerations and angular accelerations."""
    ...
    def diff_col(src: str, dst: str) -> None:
        if src not in out.columns:
            out[dst] = np.nan
            return
        values = pd.to_numeric(out[src], errors="coerce")
        deriv = values.diff() / dt
        out[dst] = deriv.replace([np.inf, -np.inf], np.nan)

    diff_col("vel_north_m_s", "derived_accel_north_m_s2")
    diff_col("vel_east_m_s", "derived_accel_east_m_s2")
    diff_col("vel_down_m_s", "derived_accel_down_m_s2")
    diff_col("roll_rate_rad_s", "derived_roll_accel_rad_s2")
    diff_col("pitch_rate_rad_s", "derived_pitch_accel_rad_s2")
    diff_col("yaw_rate_rad_s", "derived_yaw_accel_rad_s2")
    return out
...
def compute_quality_metrics(raw_df: pd.DataFrame, analysis_df: pd.DataFrame, scenario: dict[str, Any]) -> dict[str, Any]:
    ...
    ref_north = finite_series(analysis_df, "ref_north_m")
    ref_east = finite_series(analysis_df, "ref_east_m")
    ref_down = finite_series(analysis_df, "ref_down_m")
    ref_vn = finite_series(analysis_df, "ref_north_m_s")
    ref_ve = finite_series(analysis_df, "ref_east_m_s")
    ref_vd = finite_series(analysis_df, "ref_down_m_s")
    imu_accel_down = finite_series(analysis_df, "imu_accel_down_m_s2")
    derived_accel_down = finite_series(analysis_df, "derived_accel_down_m_s2")
    derived_yaw_accel = finite_series(analysis_df, "derived_yaw_accel_rad_s2")
    ...
    metrics = {
        ...
        "analysis_ref_altitude_min_m": safe_float((-ref_down).min()) if len(ref_down) else None,
        "analysis_ref_altitude_max_m": safe_float((-ref_down).max()) if len(ref_down) else None,
        "analysis_ref_north_range_m": safe_float(ref_north.max() - ref_north.min()) if len(ref_north) else None,
        "analysis_ref_east_range_m": safe_float(ref_east.max() - ref_east.min()) if len(ref_east) else None,
        "analysis_ref_north_velocity_rms_m_s": safe_float(np.sqrt(np.mean(np.square(ref_vn)))) if len(ref_vn) else None,
        "analysis_ref_east_velocity_rms_m_s": safe_float(np.sqrt(np.mean(np.square(ref_ve)))) if len(ref_ve) else None,
        "analysis_ref_down_velocity_rms_m_s": safe_float(np.sqrt(np.mean(np.square(ref_vd)))) if len(ref_vd) else None,
        ...
        "analysis_imu_accel_down_rms_m_s2": safe_float(np.sqrt(np.mean(np.square(imu_accel_down)))) if len(imu_accel_down) else None,
        "analysis_derived_accel_down_rms_m_s2": safe_float(np.sqrt(np.mean(np.square(derived_accel_down)))) if len(derived_accel_down) else None,
        "analysis_derived_yaw_accel_rms_rad_s2": safe_float(np.sqrt(np.mean(np.square(derived_yaw_accel)))) if len(derived_yaw_accel) else None,
    }
...
def plot_quicklook(raw_df: pd.DataFrame, analysis_df: pd.DataFrame, scenario: dict[str, Any], path: Path) -> None:
    ...
    if "ref_down_m" in raw_df:
        axes[0].plot(
            raw_df["time_s"],
            -pd.to_numeric(raw_df["ref_down_m"], errors="coerce"),
            "k:",
            lw=1.0,
            label="position ref alt",
        )
    ...
    if "ref_thrust" in raw_df and pd.to_numeric(raw_df["ref_thrust"], errors="coerce").notna().any():
    ...
    elif "ref_north_m" in raw_df and "ref_east_m" in raw_df:
        axes[5].plot(raw_df["time_s"], pd.to_numeric(raw_df["ref_north_m"], errors="coerce"), lw=1.0, label="ref north")
        axes[5].plot(raw_df["time_s"], pd.to_numeric(raw_df["ref_east_m"], errors="coerce"), lw=1.0, label="ref east")
        axes[5].set_ylabel("Pos Ref (m)")
        axes[5].grid(True, alpha=0.25)
        axes[5].legend(loc="best")
    elif "ref_north_m_s" in raw_df and "ref_east_m_s" in raw_df:
        axes[5].plot(raw_df["time_s"], pd.to_numeric(raw_df["ref_north_m_s"], errors="coerce"), lw=1.0, label="ref vn")
        axes[5].plot(raw_df["time_s"], pd.to_numeric(raw_df["ref_east_m_s"], errors="coerce"), lw=1.0, label="ref ve")
        axes[5].plot(raw_df["time_s"], pd.to_numeric(raw_df["ref_down_m_s"], errors="coerce"), lw=1.0, label="ref vd")
        axes[5].set_ylabel("Vel Ref (m/s)")
        axes[5].grid(True, alpha=0.25)
        axes[5].legend(loc="best")
...
async def main_async(args: argparse.Namespace) -> None:
    ...
    run_metadata = {
        "collector_version": "v16_recovery_tracking_error",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="PX4 Gazebo Phase 1 data collector v16 recovery tracking-error.")
