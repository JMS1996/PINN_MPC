#!/usr/bin/env python3
"""
PX4 Phase 1 dataset builder, v7 yaw-relative block split.
...
v7 keeps the v15 yaw-relative action contract and changes train/val/test
assignment from row-random splitting to contiguous time-block splitting. This
preserves rollout windows inside validation/test sets.
...
def make_output_dir(config: dict[str, Any], output_root: str | None) -> Path:
    ...
    name = config.get("dataset_name", "px4_phase1_closed_loop_yawrel_block_dataset_v7")
...
def main() -> None:
    ...
    write_json(
        ...
        {
            "builder_version": "v7_block_split",
