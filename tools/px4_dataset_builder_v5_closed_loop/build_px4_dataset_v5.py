#!/usr/bin/env python3
"""
PX4 Phase 1 dataset builder, v5 closed-loop.

Input:
    One or more px4_phase1_v14 run directories containing summary.csv and
...
v5 switches the action contract from open-loop attitude-rate/thrust commands to
closed-loop PX4 position/velocity/yaw offboard setpoints from v14.
...
def make_output_dir(config: dict[str, Any], output_root: str | None) -> Path:
    ...
    name = config.get("dataset_name", "px4_phase1_closed_loop_dynamics_dataset_v5")
...
def deterministic_row_split(df: pd.DataFrame, config: dict[str, Any]) -> pd.DataFrame:
    ...
    # Hard scenarios are useful for testing, but making the mixed sequence purely
    # OOD makes the rollout test harsher than this Phase 1 local-model objective.
    # Keep a holdout fraction and let the rest join train/val.
...
def main() -> None:
    ...
    parser.add_argument("--run-dir", action="append", default=None, help="Specific px4_phase1_v14 run directory. Can be repeated.")
    ...
    write_json(
        ...
        {
            "builder_version": "v5_closed_loop",
