# PX4 Phase 1 Rate-Setpoint Collection v8

Version `v8` narrows hover thrust between v7's falling `0.50` case and climbing `0.54` case.
...
```bash
cd /mnt/c/Users/jangm/Documents/New\ project/drive_upload_code/PINN_MPC/tools/px4_phase1_v8
python3 collect_px4_phase1_v8.py --config scenarios.yaml --scenario T0_thrust_cal_0510
python3 collect_px4_phase1_v8.py --config scenarios.yaml --scenario T1_thrust_cal_0520
python3 collect_px4_phase1_v8.py --config scenarios.yaml --scenario T2_thrust_cal_0530
python3 collect_px4_phase1_v8.py --config scenarios.yaml --scenario T3_thrust_cal_0525
```
...
runs = sorted(root.glob('px4_phase1_standard_vtol_rate_thrust_cal_v8_*'))
rows = []
for run in runs[-10:]:
    p = run / 'summary.csv'
    if p.exists():
...
```bash
python3 collect_px4_phase1_v8.py --config scenarios.yaml --scenario R1_pitch_rate_doublet_safe
```
