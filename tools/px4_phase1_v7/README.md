# PX4 Phase 1 Rate-Setpoint Collection v7

Version `v7` calibrates hover thrust between the falling v6 cases and the runaway-climb v4 case.
...
```bash
cd /mnt/c/Users/jangm/Documents/New\ project/drive_upload_code/PINN_MPC/tools/px4_phase1_v7
python3 collect_px4_phase1_v7.py --config scenarios.yaml --scenario T0_thrust_cal_046
python3 collect_px4_phase1_v7.py --config scenarios.yaml --scenario T1_thrust_cal_050
python3 collect_px4_phase1_v7.py --config scenarios.yaml --scenario T2_thrust_cal_054
python3 collect_px4_phase1_v7.py --config scenarios.yaml --scenario T3_thrust_cal_058
python3 collect_px4_phase1_v7.py --config scenarios.yaml --scenario T4_thrust_cal_060
```
...
runs = sorted(root.glob('px4_phase1_standard_vtol_rate_thrust_cal_v7_*'))
rows = []
for run in runs[-10:]:
    p = run / 'summary.csv'
    if p.exists():
...
```bash
python3 collect_px4_phase1_v7.py --config scenarios.yaml --scenario R1_pitch_rate_doublet_safe
```
